...
### Results table
This table displays the optimal hyperparameter values and predictive accuracy on each test fold. The hyperparameter column headings of the table will change depending on the modelling task (regression or classification). Performance of the algorithm on the train and test sets is provided in the final two columns. For regression this is the mean squared error and for classification it is the misclassification rate (proportion of cases misclassified). Performance is often slightly lower on test folds, but if there is a large discrepancy between the two it indicates a poor ability of the algorithm to generalize to new, unseen data. For reporting purposes you can provide the results on each test fold, or the mean and SD across the test folds.   

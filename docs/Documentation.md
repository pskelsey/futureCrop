...
# Model Tab
The risk models you create here must be a function of one or more of the weather variables included in the climate change projection data: temperature (deg. C), relative humidity (%), wind speed (m s<sup>-1</sup>), net surface shortwave radiation flux (W m<sup>-2</sup>), precipitation (mm d<sup>-1</sup>), total cloud (%). Iclusion of weather variables 

Your 'response variable' (or dependent variable) can be any weather-dependent process involving / affecting / occurring in crops, e.g., crop diseases, pests, crop growth etc.    

### Algorithms
The app uses the MATLAB commands ``fitrensemble`` to fit regression tree ensembles, and ``fitcensemble`` to fit classification tree ensembles, depending on the required modelling task. This is selected using 'Task' switch. If an attempt is made to upload data that do not match the selected task, a warning will be issued and the data will not be loaded. See the next section for the required data format.

### Data format and upload
Data files must be .xls or .xlsx. Data should be arranged in rows with columns for the different variables. The column header for the response (dependent) variable should be an upper case 'Y', and the column headers for your weather variables should be all lower case: temp, rh, rain, wind, rad, cloud. It doesn't matter what order your columns are in, and you can include as many of the set of 6 weather variables as required. For rgression tasks your Y variable should be continuous and for classification tasks it must be a discrete number, i.e., 1, 2, 3 as opposed to low, medium, high or any other text description or date. This is to eliminate the risk of typos, different date formats etc. These will be treated as categorical data. You ideally want at least 10 values of Y for each discrete class. Missing data should be left as blank cells - do not use missing data identifiers (e.g., -9999) - these will be ignored when training an algorithm on your data. Two 'toy datasets' are included for regression and classification, and you can refer to these if in doubt about the required data format. To upload your data, select the required type of modelling task (regression or classification) then click the 'Load' button. This will open up a dialogue box that allows you to search for your file. 

### Sampling strategy
It is standard practice in machine learning to train, tune, and test your algorithm on different portions of your data. This is because machine learning algorithms have parameters that are learned from the data (training) and hyperparameters that affect the learning process, whose value must be set before learning begins. Testing the trained and tuned algorithm on separate, unseen data ensures you get an unbiased estimate of how well the algorithm will perform when confronted with new data. This is called the 'generalization accuracy'. A number of sampling strategies are available here in the 'Sampling' list. After you upload your data a strategy will be automatically selected for you based on the size of your dataset, but you are free to change this. If you select 'none' then all the data will be used for learning, default values for the hyperparameters will be used (no tuning), and the model will be 'tested' using all the data it has already learned from. This is the least desirable option as it can lead to 'overfitting', where the algorithm learns the detail and noise in the training data to the extent that it negatively impacts performance when confronted with new data. It is, however, the fastest option. The next 3 options are prefixed by 'NCV' which stands for nested cross-validation. Cross-validation, or k-fold cross-validation, is a procedure used to estimate the performance of a machine learning algorithm when making predictions on data not used during the training of the model. The cross-validation has a single hyperparameter 'k' that controls the number of subsets that a dataset is split into. Once split, each subset is given the opportunity to be used as a test set while all other subsets together are used as a training dataset. This means that k-fold cross-validation involves fitting and evaluating k models. This, in turn, provides k estimates of a model’s performance on the dataset, which we average across the test folds. In nested CV we have two loops of CV: an inner and an outer loop. Each training set of the outer cross-validation is further subdivided into folds in an inner loop to find the best hyperparameter values for each training set. This is achieved using 'Bayesian optimization', via MATLAB's bayesopt command with k = 5. Once the optimal hyperparameter values are found, the algorithm is retrained and tested in the outer loop. This is considered to be the 'gold-standard' approach in machine learning, but it is computationally expensive and may be slow to run depending on the size of your dataset. The outer loop can be k = 5, k = 10, or 'loocv', which stands for leave-one-out cross-validation. This is an extreme version of k-fold cross-validation where k is set to the number of examples in the dataset. It requires one model to be created and evaluated for each example in your dataset, and therefore has the maximum computational cost and is not appropriate for very large datasets. Finally, there is the 'holdout' option, in which 80% of your data is used for tuning in a 5-fold Bayesian optimization, and 20% is 'held out' for testing the optimized algorithm. Automatic selection of the sampling strategy follows a commonly used approach based on the number of examples, n, in your dataset: n <= 20 = 'none', 20 < n <= 100 = loocv, 100 < n <= 1000 = 10-fold, 1000 < n <= 10,000 = 5-fold, and n > 10,000 = ''holdout'. 
